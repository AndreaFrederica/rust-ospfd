...
use crate::{constant::LsaMaxAge, util::AbortHandle};

pub struct LsaTimer {
    created: Instant,
    /// the refresh timer handle will abort automatically when the timer is dropped
    /// so we don't need to abort it manually
    _refresh: AbortHandle,
}
...
impl LsaTimer {
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        Self {
            created: Instant::now(),
            _refresh: tokio::spawn(async move {
